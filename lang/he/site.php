<?php

return array (
  'nav_home' => 'דף הבית',
  'nav_about' => 'אודות',
  'nav_courses' => 'קורסים',
  'nav_methodology' => 'מתודולוגיה',
  'nav_contact' => 'יצירת קשר',
  'nav_privacy' => 'מדיניות פרטיות',
  'nav_terms' => 'תנאי שימוש',
  'nav_login' => 'כניסה',
  'nav_dashboard' => 'לוח בקרה',
  'hero_title' => 'למד שפות עם מומחי שפה',
  'hero_subtitle' => 'מתודולוגיה מודרנית, מורים מוסמכים וחוויית לימוד שמשנה חיים.',
  'hero_cta' => 'גלה את הקורסים שלנו',
  'hero_cta2' => 'למד עוד',
  'courses_title' => 'הקורסים שלנו',
  'courses_subtitle' => 'בחר שפה והתחל את המסע שלך',
  'course_portuguese' => 'פורטוגזית',
  'course_english' => 'אנגלית',
  'course_spanish' => 'ספרדית',
  'course_hebrew' => 'עברית',
  'course_greek' => 'יוונית',
  'course_learn_more' => 'למד עוד',
  'advantages_title' => 'למה לבחור ב-Gramma?',
  'advantage_1_title' => 'דוברי שפת אם',
  'advantage_1_text' => 'למד עם דוברי שפת אם ומקצוענים מוסמכים.',
  'advantage_2_title' => 'מתודולוגיה מודרנית',
  'advantage_2_text' => 'שיעורים דינמיים, אינטראקטיביים ומותאמים לקצב שלך.',
  'advantage_3_title' => 'גמישות מלאה',
  'advantage_3_text' => 'לוחות זמנים גמישים ומסגרות פנים-אל-פנים ומקוונות.',
  'advantage_4_title' => 'הסמכה בינלאומית',
  'advantage_4_text' => 'תעודות המוכרות בינלאומית.',
  'methodology_title' => 'המתודולוגיה שלנו',
  'methodology_subtitle' => 'גישה מוכחת ללימוד שפות',
  'methodology_1_title' => 'הטמעה תרבותית',
  'methodology_1_text' => 'שקע את עצמך בשפה ובתרבות שאתה לומד.',
  'methodology_2_title' => 'תרגול שיחה',
  'methodology_2_text' => 'דבר מהשיעור הראשון עם מצבים אמיתיים.',
  'methodology_3_title' => 'הערכה מתמשכת',
  'methodology_3_text' => 'מעקב מותאם אישית אחר ההתקדמות שלך.',
  'testimonials_title' => 'מה אומרים הסטודנטים שלנו',
  'testimonial_1_text' => 'Gramma Institute שינה את הקשר שלי עם האנגלית. תוך 6 חודשים כבר תקשרתי בביטחון.',
  'testimonial_1_author' => 'מריה סילבה',
  'testimonial_2_text' => 'ללמוד עברית נראה בלתי אפשרי לפני Gramma. המתודולוגיה מדהימה!',
  'testimonial_2_author' => 'ז\'ואן פריירה',
  'testimonial_3_text' => 'מורים מצוינים, שיעורים דינמיים. אני ממליץ בלי היסוס.',
  'testimonial_3_author' => 'אנה קוסטה',
  'contact_title' => 'צור קשר',
  'contact_subtitle' => 'אנחנו מוכנים לענות על שאלותיך',
  'contact_name' => 'שם',
  'contact_email' => 'דואר אלקטרוני',
  'contact_message' => 'הודעה',
  'contact_send' => 'שלח הודעה',
  'contact_phone' => 'טלפון',
  'contact_address' => 'כתובת',
  'about_title' => 'אודות Gramma Institute',
  'about_subtitle' => 'היסטוריה של מצוינות בהוראת שפות',
  'about_mission' => 'המשימה שלנו',
  'about_vision' => 'החזון שלנו',
  'about_values' => 'הערכים שלנו',
  'about_landing_title'  => 'אודותינו',
  'about_landing_lede'   => 'חמישה פרקים המסבירים מי אנחנו, מה אנו חוקרים ומדוע השפה חשובה.',
  'about_read_more'      => 'קרא עוד',
  'about_overview'       => 'סקירה',
  'nav_partners'         => 'שותפים',
  'nav_resources'        => 'משאבים',
  'partners_page_title'  => 'שותפים',
  'partners_page_lede'   => 'המוסדות והאנשים ההולכים לצידנו במכון גרמא.',
  'partners_empty'       => 'שותפים יתווספו בקרוב.',
  'resources_page_title' => 'משאבים',
  'resources_page_lede'  => 'חומרי עזר, כלי מחקר ומשאבים מקוונים לסטודנטים, חוקרים וחובבי שפה.',
  'resources_open_link'  => 'פתח',
  'resources_back'       => 'כל המשאבים',
  'footer_rights' => 'כל הזכויות שמורות.',
  'footer_privacy' => 'מדיניות פרטיות',
  'footer_terms' => 'תנאי שימוש',
  'footer_contact' => 'יצירת קשר',
  'language_select' => 'שפה',
  'select_language' => 'בחר שפה',
  'page_not_found' => 'הדף לא נמצא',
  'go_home' => 'חזרה לדף הבית',
  'nav_glossary' => 'מילון',
  'nav_founder' => 'המייסד',
  'founder_eyebrow' => 'קולו של המייסד',
  'founder_section_title' => 'הבית שבנינו',
  'founder_read_letter' => 'קרא את המכתב המלא',
  'founder_follow' => 'עקבו אחריי',
  'founder_letter_title' => 'מכתב למי שמגיע',
  'course_back' => 'כל הקורסים',
  'course_about' => 'אודות הקורס',
  'course_history' => 'תולדות השפה',
  'course_alphabet' => 'הא״ב',
  'course_for_whom' => 'למי זה מתאים',
  'course_what_you_learn' => 'מה תלמדו',
  'course_levels' => 'הרמות',
  'course_teacher' => 'המורה',
  'course_contact_title' => 'אני רוצה לדעת יותר',
  'course_contact_text' => 'דברו עם הצוות האקדמי שלנו. אנו עונים בשעות, לא בימים.',
  'course_enroll' => 'הרשמה',
  'course_format' => 'תבנית',
  'course_duration' => 'משך כולל',
  'course_price' => 'השקעה',
  'course_size' => 'גודל הכיתה',
  'glossary_eyebrow' => 'מילה אחר מילה',
  'glossary_title' => 'מילון Gramma',
  'glossary_subtitle' => 'המילים ששינו את החשיבה — מוסברות בנחת.',
  'glossary_filter_all' => 'הכל',
  'glossary_filter_greek' => 'יוונית',
  'glossary_filter_hebrew' => 'עברית',
  'glossary_filter_featured' => 'מומלצים',
  'glossary_read_more' => 'קרא ערך',
  'glossary_etymology' => 'אטימולוגיה',
  'glossary_example' => 'דוגמה',
  'glossary_quote' => 'ציטוט קלאסי',
  'glossary_back' => 'חזרה למילון',
  'glossary_empty' => 'אין ערכים עדיין. הוסיפו אותם בלוח הבקרה.',
  'see_all_courses' => 'ראה את כל הקורסים',
  'see_glossary' => 'ראה את המילון',
  'nav_events' => 'אירועים',
  'events_eyebrow' => 'יומן אקדמי',
  'events_title' => 'אירועי Gramma',
  'events_subtitle' => 'ערבי קריאה, סדנאות והעמקות.',
  'events_upcoming' => 'הקרובים',
  'events_upcoming_title' => 'אירועים קרובים',
  'events_past' => 'ארכיון',
  'events_past_title' => 'אירועים קודמים',
  'events_none' => 'אין אירועים מתוכננים.',
  'events_other' => 'דרכים נוספות',
  'events_other_title' => 'עוד ביומן',
  'event_free' => 'חינם',
  'event_paid' => 'בתשלום',
  'event_details' => 'פרטים',
  'event_spots_left' => 'מקומות',
  'event_back' => 'כל האירועים',
  'event_when' => 'מתי',
  'event_time' => 'שעה',
  'event_where' => 'איפה',
  'event_address' => 'כתובת',
  'event_investment' => 'השקעה',
  'event_free_subtitle' => 'הרשמה חינם. מקומות מוגבלים.',
  'event_paid_subtitle' => 'תשלום בהרשמה. מקומות מוגבלים.',
  'event_register' => 'הרשמה',
  'event_join' => 'הצטרפו לחדר',
  'event_contact' => 'דברו עם הצוות',
  'event_speaker' => 'מועבר על־ידי',
  'promo_code' => 'קוד',
);
